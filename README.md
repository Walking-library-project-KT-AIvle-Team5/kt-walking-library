...
본 프로젝트는 "AI 기반 자동 출간 및 구독 플랫폼"인 'KT 걷다가서재'를 클라우드 네이티브 환경에서 개발하는 것을 목표로 합니다[2].
도메인 주도 설계(DDD)를 바탕으로 마이크로서비스 아키텍처(MSA)를 분석 및 설계하고, Spring Boot와 React를 사용하여 구현합니다[2]. 최종적으로는 Azure Kubernetes Service(AKS)에 배포하고, CI/CD 파이프라인과 모니터링 시스템을 구축하여 안정적인 운영 환경을 마련합니다[2].
...
| 사용자 | 기능 항목 | 상세 요구 사항 |
| :--- | :--- | :--- |
| **작가** | 작가 등록 | 전용 페이지에서 기본 정보, 자기소개, 포트폴리오 입력 후 등록을 신청하며, 관리자의 승인 후 작가 전용 기능에 접근합니다[2]. |
| | 글 작성 및 저장 | 웹 에디터를 통해 다양한 유형의 글을 작성하고, 임시 저장 및 최종 저장 기능을 이용할 수 있습니다[2]. |
| | AI 기반 출간 | 작성 완료 후 AI 기반 표지 이미지 생성과 책 요약 요청을 통해 이미지 및 요약문을 생성합니다[2]. |
| **구독자** | 회원 가입 및 포인트 | 신규 가입 시 기본 1,000포인트, KT 고객일 경우 추가 5,000포인트가 지급됩니다[2]. |
| | 콘텐츠 열람 | 월 9,900원 요금제 가입 시 콘텐츠 무제한 열람, 또는 포인트를 차감하여 개별 전자책을 열람할 수 있습니다[2]. |
| **플랫폼** | 콘텐츠 관리 | 작가가 출간 요청한 콘텐츠의 내용, 표지, 요약 등을 검토하고 승인 또는 반려합니다[2]. |
| | 리포트 생성 | 구독자 수, 열람 수, 포인트 사용 내역 등 주요 지표를 주간/월간 리포트로 생성합니다[2]. |
| **AI 자동화** | AI 표지 이미지 생성 | 작가가 요청 시 제목, 요약, 키워드를 기반으로 AI가 자동으로 커버 이미지를 생성합니다[2]. |
| | 카테고리/가격 자동화 | 도서 줄거리에 따라 카테고리를 자동 분류하고 적정 구독료를 산정합니다[2]. |

### 비기능적 요구사항 (Non-functional Requirements)

- **트랜잭션 (Transaction):** 작가의 출간 요청(AI 처리 포함)과 구독자의 구독(포인트 차감 포함)은 데이터 정합성을 보장하며 성공적으로 처리되어야 합니다[2].
- **장애 격리 (Fault Tolerance):** AI 출간 서비스나 포인트 서비스에 장애가 발생하더라도, '출간 요청' 및 '구독 신청' 등 핵심 기능은 정상적으로 처리되어야 합니다[2].
- **성능 (Performance):**
  - 구독 신청 내역, 서적 정보, 포인트 사용 정보는 실시간 조회가 가능해야 합니다 (**CQRS 패턴** 적용)[2].
  - AI 서비스의 실행 완료 상태 등은 해당 작가에게 실시간으로 전달되어야 합니다 (**Event-driven 아키텍처** 적용)[2].

---

## 🏛️ 시스템 아키텍처 (System Architecture)

본 프로젝트는 마이크로서비스 아키텍처를 기반으로 하며, 각 서비스는 독립적으로 개발 및 배포됩니다. 모든 외부 요청은 API Gateway를 통해 라우팅되며, 서비스 간 통신은 이벤트 기반의 비동기 방식(Kafka)으로 처리됩니다[2].
...
AI 서비스는 `curl` 명령어를 통해 직접 테스트할 수 있습니다[2].
...
서비스 간의 데이터 동기화는 **Kafka**를 통해 이벤트 기반으로 처리됩니다. 아래는 `PointCharged`, `SubscriptionActivated` 등 실제 이벤트 로그입니다[2].
<div align="center">
...
모든 마이크로서비스는 컨테이너화되어 **Azure Kubernetes Service(AKS)** 클러스터에 배포 및 운영됩니다[2].
<div align="center">
  <img src="docs/images/22.png" alt="Kubernetes Deployment Status" width="800"/>
</div>

### CI/CD 파이프라인 (CI/CD Pipeline)
**GitHub Actions**를 사용하여 소스 코드 변경 시 자동으로 빌드, 컨테이너 이미지 생성, 쿠버네티스 배포가 이루어지는 CI/CD 파이프라인을 구축했습니다[2].
<div align="center">
  <img src="docs/images/25.png" alt="CI/CD Pipeline" width="800"/>
</div>

### 통합 모니터링 및 로깅 (Monitoring & Logging)
- **Prometheus**와 **Grafana**를 이용해 서비스 상태를 실시간으로 모니터링합니다[2].
- **Loki**를 통해 분산된 마이크로서비스의 로그를 중앙에서 통합 조회 및 관리합니다[2].
<div align="center">
